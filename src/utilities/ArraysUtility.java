...
public class ArraysUtility {
    ...
    //returns the maximum for integer array
    public static int max(int[] numbers) {
        Arrays.sort(numbers);
        return numbers[numbers.length - 1];
    }

    //returns the maximum for double array
...
//{1,2,3,4}, 6
        for (int each : array) {
            if (each == element) {//if any array's element is matching with the give element,
                // it means the give elements is contained in the array
                result = true;
            }
        }
        return result;
